...
import datetime
import requests
from urllib.parse import urlencode  # Add this line to import urlencode
...
class Face_Recognizer:
    def __init__(self, email, password):
        ...
        self.email = email  # Store email as instance variable
        self.password = password  # Store password as instance variable
    ...
    def attendance(self, name):
        ...
        if existing_entry:
            ...
            # Send GET request
            response = requests.get(php_page_url)

            # Check the content of the response
            if response.text.strip() == "false":
                print("Failed to mark attendance. Exiting...")
                conn.close()
                return  # Return from the method if attendance marking fails

            if response.status_code == 200:
                print("Attendance data successfully sent to PHP page")
            else:
                print("Failed to send attendance data to PHP page")
        else:
            ...
            while stream.isOpened():
                ...
                if (self.current_frame_face_cnt == self.last_frame_face_cnt) and (
                    ...
                    for i in range(self.current_frame_face_cnt):
                     ...
                     (faces[i].left(), faces[i].bottom() + 20), cv2.FONT_HERSHEY_SIMPLEX,
...
def main():
    ...
    # Initialize Face_Recognizer class with email and password
    Face_Recognizer_con = Face_Recognizer(email, password)
